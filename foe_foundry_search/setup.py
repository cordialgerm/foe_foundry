...
def _should_rebuild_search_index() -> bool:
    ...
    monsters_time = max((f.stat().st_mtime for f in monsters_cache.iterdir()), default=0)
    documents_time = max((f.stat().st_mtime for f in documents_cache.iterdir()), default=0)
