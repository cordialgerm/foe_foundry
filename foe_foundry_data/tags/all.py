    ...
    def TagLookup(self) -> Dict[str, TagInfoModel]:
        ...
        for tag_def in ALL_TAG_DEFINITIONS:
            ...
            monsters_with_tag = [
                ...
                if monster.tags and any(tag.key == tag_def.key for tag in monster.tags)
            ]

            # Select diverse examples (up to 4)
            diverse_monsters = self._select_diverse_examples(
                monsters_with_tag, max_examples=4
            )
            ...
            # Create TagInfoModel
            tag_models[tag_def.key] = TagInfoModel.from_tag_definition(
                tag_def, example_monsters
            )
    ...
    def _select_diverse_examples(
        ...
        for creature_type in by_creature_type:
            if len(selected) >= max_examples:
                break
            candidates = [
                m for m in by_creature_type[creature_type] if m.key not in used_monsters
            ]
            if candidates:
                chosen = random.choice(candidates)
                selected.append(chosen)
