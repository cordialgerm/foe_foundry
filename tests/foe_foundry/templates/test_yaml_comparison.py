...
def test_yaml_template_exact_comparison(template: MonsterTemplate):
    ...
    for stat_key, base_stat in base_stats.items():
        assert stat_key in new_stats, f"Missing stats for key {stat_key} in YAML template"
        ...
        assert mismatch_reason is None, f"Stats mismatch for key {stat_key}: {mismatch_reason}"
...
def compare_stats(stats1: BaseStatblock, stats2: BaseStatblock) -> str | None:
    ...
    if stats1.template_key != stats2.template_key:
        return f"template_key mismatch: '{stats1.template_key}' != '{stats2.template_key}'"
    ...
    if stats1.creature_type != stats2.creature_type:
        return f"creature_type mismatch: {stats1.creature_type} != {stats2.creature_type}"
...
def test_template_coverage():
    ...
    if missing_python:
        pytest.fail(f"YAML template files missing Python templates: {sorted(missing_python)}")
    
    # All templates should have bidirectional coverage
    assert python_templates == yaml_template_keys, "Template coverage mismatch between Python and YAML implementations"
