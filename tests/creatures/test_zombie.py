...
from foe_foundry.creatures import GenerationSettings, zombie
...
@pytest.mark.parametrize(
    "settings", zombie.ZombieTemplate.generate_settings(), ids=_ids
)
def test_zombie_statblocks(settings: GenerationSettings):
    examples_dir = Path(__file__).parent.parent.parent / "examples" / "zombies"
    examples_dir.mkdir(exist_ok=True, parents=True)
    stats = zombie.ZombieTemplate.generate(settings).finalize()
