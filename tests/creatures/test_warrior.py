...
from foe_foundry.creatures import warrior
...
@pytest.mark.parametrize("args", warrior.WarriorTemplate.generate_options(), ids=_ids)
def test_warrior_statblocks(args: dict):
    examples_dir = Path(__file__).parent.parent.parent / "examples" / "warriors"
    ...
    stats = warrior.WarriorTemplate.generate(**args, rng_factory=rng_factory).finalize()
