...
class TestFacetParsing:
    ...
    def test_parse_creature_type_from_query(self):
        ...
        assert (
            parse_creature_type_from_query("some beast creature") == CreatureType.Beast
        )
    ...
    def test_is_facet_only_query(self):
        ...
        assert (
            is_facet_only_query("fire dragon") is False
        )  # Contains "dragon" but not exact match, should use text search
